...
async def draw_abyss_img(
    ...
    if floor:
        ...
        for _floor_data in raw_abyss_data["floors"]:
            ...
            if raw_abyss_data["floors"] and floor < raw_abyss_data["floors"][0]["index"]:
                return "你本期已跳过该层深渊!"
            else:
                return "你还没有挑战过该层深渊!"
    else:
    ...
    # 过滤数据
    raw_abyss_data["floors"] = [i for i in raw_abyss_data["floors"] if i["index"] >= 9]
    ...
    for num in range(4):
        ...
        if len(raw_abyss_data["floors"]) - 1 >= num:
            ...
            if _floor["settle_time"] == "0000-00-00 00:00:00":
                ...
                _timestamp = int(_floor["levels"][-1]["battles"][-1]["timestamp"])
            ...
            for index_part, battle in enumerate(level["battles"]):
                for index_char, char in enumerate(battle["avatars"]):
                    ...
                    if char["id"] in char_temp:
                        ...
                        for i in char_data:
                            if i["id"] == char["id"]:
                                talent_num = str(i["actived_constellation_num"])
