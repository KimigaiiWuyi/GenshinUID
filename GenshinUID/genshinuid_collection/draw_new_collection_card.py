...
async def _draw_explore(raw_data: IndexData):
    ...
    image = Image.new(
        ...
        (
            x,
            line * ((((len(worlds) - 1) // column) + 1) + (((len(new_culus) - 1) // column) + 1) + 1)
            + title_offer
            + div_h * 3
            + footer,
        ),
    ...
    image.paste(
        ...
        (
            0,
            div_h + line * (((len(new_culus) - 1) // column) + 1) + title_offer,
        ),
        div_b,
    )
    ...
    for index_c, chest_name in enumerate(CHEST_):
        ...
        image.paste(
            ...
            (
                ...
                +div_h * 2 + line * (((len(new_culus) - 1) // column) + 1) + line * (index_c // column) + title_offer,
            ),
            area_bg,
        )
    ...
    image.paste(
        ...
        (
            0,
            div_h * 2 + line * (((len(new_culus) - 1) // column) + 2) + title_offer,
        ),
        div_c,
    )
    ...
    for index, world in enumerate(worlds):
        ...
        image.paste(
            ...
            (
                ...
                +div_h * 3 + line * (((len(new_culus) - 1) // column) + 2) + line * (index // column) + title_offer,
            ),
            area_bg,
        )
...
async def draw_area(
    ...
    if offerings:
        ...
        sub_color = get_color(odata["level"], CMAP.get(oname, [10, 8, 6, 4, 2]))
