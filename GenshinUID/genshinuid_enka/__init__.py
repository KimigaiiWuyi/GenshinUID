...
@sv_get_enka.on_command(
    ...
    当用户说「圣遗物仓库」「看看圣遗物仓库」且要看图时调用。
    不要用 get_user_genshin_artifacts 代替本命令。需要已绑定 UID。
...
@sv_get_enka.on_prefix(
    ...
    用户要「看」某角色时必须走本命令，不要改用 get_user_genshin_char_detail。
    适用：「看看我的胡桃」「我的雷神面板」「查询 胡桃」「公子练度图」。

    Args:
        text: 角色名称，例如 "胡桃"、"公子"、"甘雨"
              支持昵称，例如 "雷神"、"核桃"、"小草神"
              可加"换"后缀切换武器，例如 "公子换"
...
@sv_get_enka.on_command(
    ...
    当用户说「刷新面板」「强制刷新」，或更换游戏内展柜角色后需要同步缓存时调用。
    未绑 Cookie 时只能拉到展柜角色（最多 12 名），不是全角色箱。
...
@sv_get_enka.on_command(
    ...
    当用户说「角色橱窗」「展柜角色」「展柜里有谁」且要看图时调用。
    图上是当前展柜缓存，不是账号全部角色。需要已绑定 UID。
