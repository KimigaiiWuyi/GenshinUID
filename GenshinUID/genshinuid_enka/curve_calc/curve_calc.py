...
async def get_weight_temp(prop: dict, attr: str) -> List[float]:
    weight = []
    if "攻击" in attr:
        weight.append((prop["atk_green"] / prop["baseAtk"]) * 100 / WEIGHT_MAP["百分比攻击力"])
    elif "生命" in attr:
        weight.append((prop["hp_green"] / prop["baseHp"]) * 100 / WEIGHT_MAP["百分比血量"])
    elif "防御" in attr:
        weight.append((prop["def_green"] / prop["baseDef"]) * 100 / WEIGHT_MAP["百分比防御力"])
    elif "精通" in attr:
...
async def draw_char_curve_data(char_name: str, raw_data: dict) -> Tuple[Image.Image, int]:
    ...
    for col_index, col in enumerate(CHAR_CURVE[char_name]):
        ...
        for index, i in enumerate(CURVE[col]):
            if index >= X_MAX:
                break
            x, y = (X_D / X_MAX) * index + lu_point[0], (Y_D - (Y_D / Y_MAX) * i) + lu_point[1]
        ...
        for wight in wight_temp_dict[CHAR_CURVE[char_name][col]]:
            ...
            if CHAR_CURVE[char_name][col] not in wight_point_dict:
                ...
                wight_point_dict[CHAR_CURVE[char_name][col]]["point"].append((w_x, w_y))
    ...
    for attr in wight_point_dict:
        ...
        for index, point in enumerate(wight_point_dict[attr]["point"]):
            ...
            img.paste(point_img, (int(point[0] - 15), int(point[1] - 15)), point_img)
            img_draw.line([point, (point[0], rd_point[1])], width=1, fill=(255, 255, 255))
