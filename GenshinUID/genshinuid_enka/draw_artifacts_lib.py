...
async def draw_lib(ev: Event, uid: str, num: int) -> Union[bytes, str]:
    ...
    if data is None:
        return f"你还没有圣遗物数据...请尝试使用[{PREFIX}刷新圣遗物仓库]获取数据!"
    ...
    use_percent = Decimal(can_use * 100).quantize(Decimal("0.00")) / Decimal(all_num)
    high_percent = Decimal(high_cv * 100).quantize(Decimal("0.00")) / Decimal(all_num)
    ...
    bg_draw.text((650, 141 + yo * 2), f"{high_value}", "white", gs_font_38, "mm")
    bg_draw.text((650 + xo, 141 + yo * 2), f"{high_cv}", "white", gs_font_38, "mm")
    bg_draw.text(
        (650 + xo * 2, 141 + yo * 2),
        f"{high_percent:.2f}%",
        "white",
    ...
    extra_notice = f"可用 圣遗物仓库{num + 1} 命令查看第{num + 1}页" if num < all_page else "暂无更多页数"
