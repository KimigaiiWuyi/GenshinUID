...
async def _get_single_artifact_info(aritifact: Dict) -> Image.Image:
    ...
    artifacts_img = (
        Image.open(REL_PATH / "{}.png".format(aritifact["aritifactName"])).resize((128, 128)).convert("RGBA")
    )
    ...
    bg_draw.rounded_rectangle((269, 22, 340, 42), fill=artifactsScore_color, radius=8)
...
async def get_weapon_img(char: Character) -> Image.Image:
    ...
    if char_card_path.exists():
        char_card = Image.open(CHAR_NAMECARDPIC_PATH / f"{char_id}.png").convert("RGBA").resize((560, 268))
    else:
    ...
    if len(card_prop["weaponInfo"]["weaponStats"]) == 2:
        ...
        weapon_sub_icon = Image.open(TEXT_PATH / "icon" / f"{weapon_sub_info}.png")
...
async def get_char_img(char: Character):
    ...
    for talent_num in range(0, 6):
        if talent_num + 1 <= len(card_prop["talentList"]):
            ...
            talent_img_new = talent_img.resize((40, 40), Image.Resampling.LANCZOS).convert("RGBA")
            for _ in range(2):
                char_pic.paste(
                    talent_img_new,
...
async def draw_all_char_list(uid: str) -> Union[str, bytes]:
    ...
    for index, cimg in enumerate(char_result):
        bg.paste(cimg, (25 + (index % 4) * 950, 25 + (index // 4) * 1280), cimg)
