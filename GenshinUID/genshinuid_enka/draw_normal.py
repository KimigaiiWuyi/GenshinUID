...
async def get_char_card_base(char: Character) -> Image.Image:
        ...
        if talent_num + 1 <= len(card_prop["talentList"]):
            ...
            try:
                if char.char_id in CharId2TalentIcon_data:
                    icon_name = CharId2TalentIcon_data[char.char_id][talent_num]
                else:
                    icon_name = talent["talentIcon"]
                talent_img = Image.open(ICON_PATH / f"{icon_name}.png")
            except Exception:
                talent_img = Image.open(ICON_PATH / "UI_Talent_S_Kazuha_02.png")
    ...
    for skill_num, skill in enumerate(skillList[0:2] + [skillList[-1]]):
        ...
        char_info_1.paste(skill_img_new, (78, 756 + 101 * skill_num), skill_img_new)
    ...
    if len(card_prop["weaponInfo"]["weaponStats"]) == 2:
        ...
        weapon_sub_value = card_prop["weaponInfo"]["weaponStats"][1]["statValue"]
        char_info_text.text(
            (420, 801),
            weapon_sub_info,
            (255, 255, 255),
...
async def get_bg_card(char_element: str, ex_len: int, char_img: Image.Image) -> Image.Image:
...
async def _get_single_artifact_img(aritifact: Dict) -> Image.Image:
    ...
    artifacts_piece_img = Image.open(REL_PATH / "{}.png".format(aritifact["aritifactName"]))
    artifacts_piece_new_img = artifacts_piece_img.resize((90, 90), Image.Resampling.LANCZOS).convert("RGBA")

    artifacts_img.paste(artifacts_piece_new_img, (26, 32), artifacts_piece_new_img)
    ...
    if len(aritifact["aritifactName"]) <= 8:
        ...
        main_name = aritifact["aritifactName"][:4] + aritifact["aritifactName"][4:]
    artifacts_text.text(
        (124, 51),
        main_name,
        (255, 255, 255),
    ...
    artifacts_text.rounded_rectangle((121, 99, 193, 119), fill=artifactsScore_color, radius=8)
    artifacts_text.rounded_rectangle((200, 99, 272, 119), fill=cv_color, radius=8)
...
async def get_single_artifact_img(aritifact: Dict, char: Character) -> Image.Image:
