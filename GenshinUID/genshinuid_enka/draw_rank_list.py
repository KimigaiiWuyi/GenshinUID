...
async def draw_rank_img(ev: Event, uid: str) -> Union[bytes, str]:
    ...
    for index, char in enumerate(_rank_data):
        ...
        weapon_card = await draw_weapon_card(data["weapon"]["icon"], data["weapon"]["rarity"])
        ...
        tag_weapon = Image.open(TEXT_PATH / f"char_card_{data['weapon']['refinement']}.png").resize((40, 40))
        ...
        if index % 2 == 0:
            ...
            char_draw.text((880, 39), f"数据最后更新于 {time}", grey, gs_font_15, "rm")
            char_draw.text((174, 37), f"{char_name}", "white", gs_font_32, "lm")
            ...
            char_draw.text((716, 257), str(int(float(result))), "white", gs_font_44, "lm")
            ...
            char_draw.text((760, 37), f"{char_name}", "white", gs_font_32, "rm")
            ...
            char_draw.text((237, 249), str(int(float(result))), "white", gs_font_44, "rm")
