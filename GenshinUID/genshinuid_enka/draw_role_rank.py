...
async def draw_role_rank_img(char_name: str, player_uid: Optional[str] = None) -> Union[str, bytes]:
    ...
    user_pic = await draw_pic_with_ring(Image.open(CHAR_PATH / f"{char_id}.png"), 314)
    title.paste(user_pic, (318, 57), user_pic)
    img.paste(title, (0, 0), title)

    img_draw = ImageDraw.Draw(img)
    img_draw.text((475, 425), f"{tag} / 总数据 {count}条", "white", gs_font_26, "mm")
    ...
    for index, char in enumerate(data):
        ...
        if index == 0:
            ...
            rank = int(_rank[1:]) if isinstance(_rank, str) and _rank.startswith("~") else int(_rank)
        else:
            rank += 1
