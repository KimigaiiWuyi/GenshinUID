...
async def get_buff_list(
    ...
    # 计算圣遗物buff
    if equipSets["type"] == "4":
        all_effect.append(artifact_effect_map[equipSets["set"]][f"{type}_effect"]["2"])
        all_effect.append(artifact_effect_map[equipSets["set"]][f"{type}_effect"]["4"])
        ...
        for i in artifact_effect_map:
            if i.startswith(t):
                all_effect.extend(artifact_effect_map[i][f"{type}_effect"]["2"].split(";"))
            elif i.endswith(e):
                all_effect.extend(artifact_effect_map[i][f"{type}_effect"]["2"].split(";"))
        ...
        if with_talent:
            for talent in char_effect_map[char_name][main][f"{type}_talent"]:
                if len(raw_data["talentList"]) >= int(talent):
                    all_effect.append(char_effect_map[char_name][main][f"{type}_talent"][talent])
        # 计算角色buff
        for skill in char_effect_map[char_name][main][f"{type}_skill"]:
            if char_level >= int(skill):
                all_effect.append(char_effect_map[char_name][main][f"{type}_skill"][skill])
