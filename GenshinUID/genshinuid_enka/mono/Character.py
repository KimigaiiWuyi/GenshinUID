...
class Character:
    ...
    async def new(
        ...
        if not gsconfig.get_config("OldPanle").data:
            self.card_prop = await self.get_card_prop(weapon, weapon_affix, talent_num)
        if self.card_prop == {}:
        ...
        self.rarity = await avatar_id_to_char_star(str(self.card_prop["avatarId"]))
    ...
    async def get_card_prop(
        ...
        if weapon:
            ...
            if isinstance(weapon_raw_data, int) or isinstance(weapon_raw_data, List) or weapon_raw_data is None:
                ...
                if isinstance(weapon_level_data, int) or isinstance(weapon_level_data, List):
                    return {}
            ...
            weapon_info["weaponStats"][0]["statValue"] = round(weapon_level_data["attack"])
            if weapon_raw_data["mainStatText"] != "":
                weapon_info["weaponStats"][1]["statName"] = weapon_raw_data["mainStatText"]
                if weapon_raw_data["mainStatText"] == "元素精通":
                    fake_value = round(weapon_level_data["specialized"])
                else:
                    fake_value = float("{:.2f}".format(weapon_level_data["specialized"] * 100))
                weapon_info["weaponStats"][1]["statValue"] = fake_value

            if "effectTemplateRaw" in weapon_raw_data:
                weapon_info["weaponEffect"] = weapon_raw_data["effectTemplateRaw"].format(
                    *weapon_raw_data["r{}".format(str(weapon_info["weaponAffix"]))]["values"]
                )
        ...
        part_effects.extend(await get_buff_list(self.card_prop, "normal", False))

        fight_prop_part = await self.get_effect_prop(deepcopy(fight_prop), part_effects, self.char_name)
        fight_prop_all = await self.get_effect_prop(deepcopy(fight_prop), all_effects, self.char_name)
    ...
    async def get_base_prop(self, char_name: str, char_level: int) -> Dict:
        # 武器基本属
        weapon_atk = self.card_prop["weaponInfo"]["weaponStats"][0]["statValue"]
        if len(self.card_prop["weaponInfo"]["weaponStats"]) > 1:
            weapon_sub = self.card_prop["weaponInfo"]["weaponStats"][1]["statName"]
            weapon_sub_val = self.card_prop["weaponInfo"]["weaponStats"][1]["statValue"]
        else:
            weapon_sub = ""
            weapon_sub_val = 0
        ...
        if isinstance(char_data, List) or isinstance(char_data, int) or char_data is None:
            return {}
            ...
            for prop_limit in ["A", "B", "C", "E", "Q"]:
                if weapon_type == "法器" or char_name in CHANGE_LIST:
                    ...
                    if prop_limit in ["A", "C"]:
                        prop["{}_dmgBonus".format(prop_limit)] = prop["physicalDmgBonus"]
        ...
        for effect in new_effect_list:
            ...
            if base_check:
                if effect_base in ["hp", "atk", "def"]:
                    base_effect_list.append([effect_limit, effect_attr, effect_value, effect_base])
                    continue
    ...
    async def get_base_value(self, prop: Dict) -> Dict:
        ...
        for prop_limit in ["A", "B", "C", "E", "Q"]:
            for attr in ["hp", "atk", "def"]:
                ...
                prop[f"{prop_limit}_{attr}"] = (prop[f"{prop_limit}_add{attr_up}"] + 1) * prop[f"base{attr_up}"] + prop[
                    f"ex{attr_up}"
                ]
        return prop
    ...
    async def get_fight_prop(self) -> Dict:
            ...
            if self.char_name == "钟离":
                self.power_list["E总护盾量"] = {
                    ...
                    "value": [
                        ...
                        for index, i in enumerate(self.power_list["E护盾基础吸收量"]["value"])
                    ],
                }
                ...
                for power_name in [
                    ...
                    for index, i in enumerate(self.power_list["A霜华矢命中伤害"]["value"]):
        ...
        if self.char_name in EXTRA_CHAR_LIST:
            if self.char_name == "雷电将军":
                ...
                skill_effect = EXTRA_CHAR_LIST[self.char_name]["Q每层能量伤害"]["value"]
                attack_type = "Q"
                skill_level = prop[f"{attack_type}_skill_level"] - 1
                ...
                if self.char_talent >= 1:
                    ...
                    char_talent = 0
                skill_effect = EXTRA_CHAR_LIST[self.char_name][f"E灭净三业伤害提升{char_talent}"]["value"]
                attack_type = "E"
                skill_level = prop[f"{attack_type}_skill_level"] - 1
                value = float(skill_effect[skill_level])
        ...
        # 计算全部的buff，添加入属性
        self.fight_prop = await self.get_effect_prop(deepcopy(prop), all_effect, self.char_name)
        ...
        if self.rarity != "5" and self.char_name != "香菱":
            ...
            self.without_talent_fight = await self.get_effect_prop(deepcopy(prop), part_effect, self.char_name)
