...
class Fight:
    ...
    # 进行单人伤害计算
    async def get_dmg_dict(self, char_name: str, without_talent: bool = False) -> Dict:
        result = {}
        char = self.char_list[char_name]
        # 获取本次攻击的类型
        if without_talent:
    ...
    # 伤害类型
    async def get_dmg_type(self, char: Character, seq: Optional[Dict] = None) -> Element:
        # TODO 获取本次攻击的元素
    ...
    async def get_new_fight_prop(self, char: Character) -> Dict:
        ...
        if effect_list:
            char.real_prop = await char.get_effect_prop(deepcopy(char.fight_prop), effect_list, char.char_name)
            ...
            if reaction in char.power_list[char.power_name]["name"]:
                if reaction == "蒸发":
                    ...
                    if char.char_element == "Pyro":
                        ...
                        k = 1.5
                reaction_add_dmg = k * (1 + (2.78 * em_cal) / (em_cal + 1400) + char.real_prop["a"])
                break
        else:
            reaction_add_dmg = 1
    ...
    async def get_quicken_dmg(self, char: Character) -> float:
        ...
        for reaction in ["超激化", "蔓激化"]:
            if reaction in power_name:
                ...
                power_times = 1
                if "*" in power_name:
                    power_times = float((power_name.split("*")[-1].replace(")", "")))
                quicken_dmg = (k * base_value_list[char_level - 1] * (1 + (5 * em_cal) / (em_cal + 1200))) * power_times
                break
        return quicken_dmg
    ...
    # 聚变反应
    async def get_transform_dmg(self, char: Character) -> Tuple[float, float, float]:
        ...
        if "绽放)" in char.power_name:
            ...
            if "烈绽放" in char.power_name:
                ...
                base_time = 4
            base_area = (
                base_value_list[char.char_level - 1] * base_time * (1 + (16.0 * em) / (em + 2000) + char.real_prop["a"])
            )
            is_crit = True
    ...
    async def get_dmg(
        ...
        avg_dmg = (
            normal_dmg
            if critrate < 0
            else (crit_dmg if critrate > 1 else crit_dmg * critrate + (1 - critrate) * normal_dmg)
        )
