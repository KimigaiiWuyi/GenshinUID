...
async def enka_to_card(uid: str, enka_data: Optional[EnkaData] = None) -> Union[str, bytes, Tuple[bytes, List[Dict]]]:
    try:
...
async def draw_enka_card(
    ...
    if char_list:
        ...
        for char in char_list:
            char_data_list.append({"avatarName": char, "avatarId": await name_to_avatar_id(char)})
...
async def draw_char_card(char_id: str) -> Image.Image:
    ...
    char_img = Image.open(CHAR_PATH / f"{char_id}.png").convert("RGBA").resize((204, 204))
