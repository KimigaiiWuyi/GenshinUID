...
async def enka_to_dict(uid: str, enka_data: Optional[EnkaData] = None) -> Union[List[dict], str]:
    """
    :说明:
    ...
    # 保存基本玩家信息
    async with aiofiles.open(path / f"{uid}.json", "w", encoding="UTF-8") as file:
        await file.write(json.dumps(playerInfo, indent=4, ensure_ascii=False))

    # 保存原始数据
    async with aiofiles.open(path / "rawData.json", "w", encoding="UTF-8") as file:
        await file.write(json.dumps(enka_data, indent=4, ensure_ascii=False))

    if enka_data is None or enka_data == {} or "avatarInfoList" not in enka_data or not enka_data["avatarInfoList"]:
    ...
    if not all_artifacts_path.exists():
        ...
        async with aiofiles.open(all_artifacts_path, "r", encoding="UTF-8") as file:
    ...
    for char in enka_data["avatarInfoList"]:
        ...
        weapon_info["weaponType"] = weaponHash2Type[str(weapon_info["nameTextMapHash"])]
        weapon_info["weaponName"] = weaponHash2Name[str(weapon_info["nameTextMapHash"])]
        ...
        if (
            ...
            effect = effect_raw[f"r{weapon_info['weaponAffix']}"]["description"]
        else:
            effect = "无特效。"
        ...
        # 保存原始数据
        async with aiofiles.open(path / "artifacts.json", "w", encoding="UTF-8") as file:
            await file.write(json.dumps(all_artifacts_data, indent=4, ensure_ascii=False))
        ...
        async with aiofiles.open(path / "{}.json".format(avatarName), "w", encoding="UTF-8") as file:
...
async def _get_data(
    ...
    for _artifact in artifacts_info:
        await input_artifacts_data(deepcopy(_artifact), all_artifacts_data, avatarId, char_data)
...
async def input_artifacts_data(
    ...
    artifact_temp = await get_artifact_score_data(artifact_temp, Character(char_data))
    # 加入圣遗物数据列表
    if artifact_temp not in all_artifacts_data["data"][artifact_temp["aritifactSetPiece"]]:
        all_artifacts_data["data"][artifact_temp["aritifactSetPiece"]].append(artifact_temp)
        all_artifacts_data["tag"][artifact_temp["aritifactSetPiece"]].append([avatarId])
    elif (
        avatarId
        not in all_artifacts_data["tag"][artifact_temp["aritifactSetPiece"]][
            all_artifacts_data["data"][artifact_temp["aritifactSetPiece"]].index(artifact_temp)
        ]
    ):
