...
async def _draw_card(
    ...
    card_img_draw.text(text_point, f"{gacha_num}抽", text_color, gs_font_24, "mm")
    if is_up:
...
async def draw_gachalogs_img(uid: str, ev: Event) -> Union[bytes, str]:
    ...
    for i in all_gacha_type_name:
        ...
        for index, data in enumerate(data_list[::-1]):
            ...
            if index == len(data_list) - 1:
                total_data[i]["all_time"] = (
                    ...
                    - datetime.datetime.strptime(total_data[i]["time_range"], "%Y-%m-%d %H:%M:%S")
                ).total_seconds()
                total_data[i]["time_range"] += "~" + data["time"]
            ...
            if index != 0:
                ...
                temp_time = datetime.datetime.strptime(data["time"], "%Y-%m-%d %H:%M:%S")

            # 如果这是个五星
            if "rank_type" not in data:
                if data["item_type"] == "武器":
                    data["rank_type"] = str(weaponList[data["item_id"]]["rank"])
                elif data["item_type"] == "角色":
                    data["rank_type"] = "5" if str(charList[data["item_id"]]["rank"]) == "QUALITY_ORANGE" else "4"
        ...
        if len(total_data[i]["normal_list"]) == 0:
            ...
            total_data[i]["avg"] = float("{:.2f}".format(sum(total_data[i]["r_num"]) / len(total_data[i]["r_num"])))
        # 计算平均up数量
        ...
        if f"{CHANGE_MAP[i]}_gacha_num" not in gacha_data:
            gacha_data[f"{CHANGE_MAP[i]}_gacha_num"] = len(gacha_data["data"][i])
