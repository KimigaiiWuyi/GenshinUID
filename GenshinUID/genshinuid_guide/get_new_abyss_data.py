...
async def download_Oceanid():
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get("https://enka.network/ui/UI_MonsterIcon_Oceanid_Underling.png")
...
async def get_half_img(data: List, half: Literal["Upper", "Lower"]):
    ...
    for wave in data:
        ...
        wave_tag_draw.text((36, 20), f"第{index + 1}波", (210, 210, 210), gs_font_24, "lm")
        if "ExtraDesc" in wave:
            ExtraDesc: str = wave["ExtraDesc"]["CH"]
            ExtraDesc = " > " + ExtraDesc.replace("<b>", "").replace("</b>", "")
            half_draw.text((150, 65 + temp), ExtraDesc, (210, 210, 210), gs_font_24, "lm")
        ...
        for m_index, monster in enumerate(monsters):
            ...
            monster_icon = await get_ambr_icon("monster", icon_name, MONSTER_ICON_PATH)
            ...
            monster_draw.text((160, 85), monster_name[:7], "white", gs_font_28, "lm")
            monster_draw.text(
                (160, 115),
                f"x{monster_num}",
                (210, 210, 210),
    ...
    upper_bg_draw.rounded_rectangle((20, 30, 1080, upper_h - 20), 10, (16, 13, 13, 120))
    half_img = half_img.crop((0, 0, 1100, upper_h))
    upper_bg.paste(half_img, (0, 0), half_img)
    return half_img


async def _get_data_from_url(url: str, path: Path, expire_sec: Optional[float] = None) -> Dict:
    time_difference = 10
...
async def get_review_data(version: str = Genshin_version[:3], floor: str = "12"):
    ...
    for f_index, level in enumerate(floor_monster):
        ...
        level_img = Image.new("RGBA", (1100, upper_h + lower_h + 70), (0, 0, 0, 0))
