...
async def check_today(uid: str) -> bool:
    ...
    month = now.month
    calendar_role: List[RoleCalendar] = calendar["calendar_role_infos"][str(month)]["calendar_role"]
    ...
    for bot_id in gss.active_bot:
        ...
        for user in user_list:
            if user.draw_switch != "off" and user.uid:
                tasks.append(single_get_draw(user.bot_id, user.uid, user.sign_switch, user.user_id))
            if len(tasks) >= 1:
