...
async def _draw_task_img(
    ...
    char_pic = (
        Image.open(side_path).convert("RGBA").resize((115, 115), Image.Resampling.LANCZOS)  # type: ignore
    )
...
async def get_resin_img(bot_id: str, user_id: str):
    try:
        ...
        # 开始绘图任务
        task = []
        img = Image.new("RGBA", (700 * len(useable_uid_list), 1300), (0, 0, 0, 0))
    ...
    data = FakeResin(
        **data,
        ...
        transformer=Transformer(
            obtained=True,
            recovery_time=RecoveryTime(Day=-99, Hour=1, Minute=1, Second=1, reached=False),
            wiki="",
            noticed=False,
            latest_job_id="123",
        ),
...
async def draw_bar(status: str = "un", text: str = "洞天宝钱", data: str = "未知") -> Image.Image:
...
async def draw_resin_img(uid: str) -> Image.Image:
    ...
    resin_recovery_time = await seconds2hours(int(daily_data["resin_recovery_time"]))
    ...
    for _ in range(5):
        if len(daily_data["expeditions"]) < 5:
            daily_data["expeditions"].append(Expedition(avatar_side_icon="", status="Ongoing", remained_time=233))
        else:
            break
