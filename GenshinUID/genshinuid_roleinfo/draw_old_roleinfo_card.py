...
async def _draw_char_full_pic(img: Image.Image, char_data: MihoyoAvatar, index: int):
...
async def draw_pic(uid: str) -> Union[str, bytes]:
    ...
    char_hang = 1 + (char_num - 1) // 4 if char_num > 8 else (char_num // 2) + (char_num % 2)
    ...
    char_import = Image.open(CHAR_STAND_PATH / f"{char_datas[0]['id']}.png").convert("RGBA")
    ...
    for world_part in world_exp:
        ...
        for offering in world_part["offerings"]:
            temp["extra"].append({"name": offering["name"], "level": offering["level"]})

        # 岚丹特色怪物计数
        count = 1
        for offering in world_part["boss_list"]:
            temp["extra"].append({"name": offering["name"], "level": offering["kill_num"]})
            count += 1
            if count >= 3:
                break
    ...
    if "homes" in raw_data and raw_data["homes"] and "comfort_num" in raw_data["homes"][0]:
        world_list.append(
            {
                "id": 1000,
    ...
    logger.info(t("log.genshinuid.msg_b10c11"))
    return res
    ...
    for offering_index, offering in enumerate(world["extra"]):
        if offering["name"] == "等阶":
            ...
            text_draw.text(
                (
                    260 + world_index % 4 * offset_x,
                    (len(world["exp"]) - 1) * 28 + 696 + world_index // 4 * offset_y + offering_index * 23,
                ),
...
async def _draw_char_8_pic(img: Image.Image, char_data: MihoyoAvatar, index: int):
    """
    绘制8人角色图片
    """
    ...
    # 角色立绘
    char_stand_img = Image.open(CHAR_STAND_PATH / f"{char_data['id']}.png").convert("RGBA")
    char_stand_img.putalpha(char_stand_img.getchannel("A").point(lambda x: round(x * 0.4) if x > 0 else 0))
    ...
    for equip_index, equip in enumerate(char_data["reliquaries"]):
        ...
        equip_pic = Image.open(REL_PATH / f"{equip['name']}.png").convert("RGBA").resize((50, 50))
        char_card_img.paste(equip_bg, (242 + equip_index * 50, 170), equip_bg)
        char_card_img.paste(equip_pic, (242 + equip_index * 50, 170), equip_pic)
    char_draw = ImageDraw.Draw(char_card_img)
    char_draw.text(
        (254, 36),
        f"Lv{char_data['level']}",
