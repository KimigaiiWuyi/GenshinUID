...
async def refresh(order: MysOrder, uid: str, order_id: str) -> Union[str, bytes]:
    times = 0
    while True:
...
async def topup_(
    ...
    try:
        ...
        # 物品名字(非月卡)
        item_name = item_name_full if int(goods_data["goods_unit"]) > 0 else goods_data["goods_name"]
        # 物品名字
        ...
        timestamp = strftime("%Y-%m-%d %H:%M:%S", localtime(int(item_create_time)))  # 年月日时间
