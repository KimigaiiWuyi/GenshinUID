...
async def createurl():
    ...
    res = await aiorequests.post(
        ...
        json=data, proxies={"http": None, "https": None})
    #print(res.text)
...
async def main_bot(bot,sid,userid,gid):
    ...
    if sta==0:
        ...
        stoken_data=await get_stoken(data={
...
if __name__ == '__main__':
    async def main():
        ...
        if sta==0:
            ...
            print(f"stoken_v2={stoken_data['data']['token']['token']};stuid={stoken_data['data']['user_info']['aid']};mid={stoken_data['data']['user_info']['mid']};cookie_token={cookie_token['data']['cookie_token']};")
        else:
            print("game_token获取失败")
    asyncio.run(main())
#input("按回车键退出")
