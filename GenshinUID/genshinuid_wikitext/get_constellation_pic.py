...
async def get_single_constellation_img(name: str, num: int) -> Union[str, bytes]:
    data = await get_constellation_info(name)
    if isinstance(data, int):
        return await get_error_img(data)
    else:
...
async def draw_constellation_wiki_img(data: CharacterConstellations, char_data: Character) -> bytes:
