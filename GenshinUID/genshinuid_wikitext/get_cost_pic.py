...
async def draw_single_cost(title: str, data: Dict) -> Tuple[Image.Image, str]:
    ...
    wiki_cost_bg = Image.open(TEXT_PATH / "wiki_weapon_cost.png").resize((900, 255))
    ...
    for index, cost_name in enumerate(data):
        ...
        if isinstance(material, int):
            ...
            if (
                material["typeText"] == "武器突破素材" or material["typeText"] == "角色天赋素材"
...
async def draw_char_cost_img(data: Character, talent_data: CharacterTalents):
    ...
    cost2, pos2 = await draw_single_cost("天赋素材消耗", {i: talent_costs[i] * 3 for i in talent_costs})
