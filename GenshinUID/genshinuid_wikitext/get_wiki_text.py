...
def _piece_text(piece: Mapping[str, object], key: str) -> str:
    if key not in piece:
        raise KeyError(key)
    value = piece[key]
...
async def artifacts_wiki(name: str) -> str:
    ...
    if isinstance(data, int):
        ...
        im = artifacts_im.format(
            ...
            data["effect2Pc"],
            data["effect4Pc"],
            _piece_text(data["flower"], "name"),
            _piece_text(data["flower"], "description"),
            _piece_text(data["plume"], "name"),
            _piece_text(data["plume"], "description"),
            _piece_text(data["sands"], "name"),
            _piece_text(data["sands"], "description"),
            _piece_text(data["goblet"], "name"),
            _piece_text(data["goblet"], "description"),
            _piece_text(data["circlet"], "name"),
            _piece_text(data["circlet"], "description"),
        )
    return im
...
async def weapon_wiki(name: str) -> str:
    ...
    if data["effectName"] != "":
        ...
        effect = ""
    return weapon_im.format(name, _type, star, info, atk, sub, effect)
...
async def weapon_stats_wiki(name: str, stats: int):
    ...
    if isinstance(data, int) or isinstance(data2, int):
        return "该武器不存在。"
    if isinstance(data, list) or isinstance(data2, list):
        return "请输入具体的武器名称..."
    if data is None or data2 is None:
        return "该武器不存在。"
    if data["mainStatText"] != "":
        sp = (
            data["mainStatText"] + ": " + "%.1f%%" % (data2["specialized"] * 100)
            if data["mainStatText"] != "元素精通"
            else data["mainStatText"] + ": " + str(math.floor(data2["specialized"]))
        )
    else:
        sp = ""
    im = (
...
async def char_wiki(name: str) -> str:
    ...
    cv = data["cv"]["chinese"]
    info = data["description"]
    return char_info_im.format(name, star, _type, element, up_val, bdday, polar, cv, info)
...
async def char_stats_wiki(name: str, stats: int):
    ...
    if data is None or data2 is None:
        return "该角色不存在。"
    sp = (
        data2["substatText"] + ": " + "%.1f%%" % (data["specialized"] * 100)
        if data2["substatText"] != "元素精通"
        else data2["substatText"] + ": " + str(math.floor(data["specialized"]))
    )
    im = (
        data2["name"]
        + "\n等级: "
...
async def talent_wiki(name: str, level: int) -> Union[List[Message], str]:
    talents = await get_talent_info(name)
    if isinstance(talents, int) or talents is None:
        return "该角色不存在。"
    if int(level) <= 3:
        if level == 1:
            combat = talents["combat1"]
        elif level == 2:
            combat = talents["combat2"]
        else:
            combat = talents["combat3"]
        skill_name = combat["name"]
        skill_info = combat["description"]
        skill_detail = ""
        ...
        labels = "".join(combat["attributes"]["labels"])
        ...
        for para in combat["attributes"]["parameters"]:
            if para in labels:
                label_str = labels[para]
                for index, j in enumerate(combat["attributes"]["parameters"][para]):
                    if add_switch:
                        parameters.append({})
        ...
        for k in combat["attributes"]["labels"]:
        ...
        mes_list.append(MessageSegment.text(f"【{skill_name}】\n{skill_info}"))
    ...
    if level == 4:
        passive = talents["passive1"]
    elif level == 5:
        passive = talents["passive2"]
    elif level == 6:
        passive = talents["passive3"]
    else:
        if "passive4" not in talents:
            return "该角色未有第四个被动天赋..."
        passive = talents["passive4"]
