...
async def panle2Json() -> None:
    ...
    """
    wb = openpyxl.load_workbook(str(DATA_PATH / "参考面板4.4A.xlsx"), data_only=True)
    ...
    for row in range(9, 410):
        ...
        if char_name and char_name != "角色" and isinstance(char_name, str):
            ...
            char_name = char_name.replace("空/荧", "旅行者").replace("（", "(").replace("）", ")")
            ...
            if char_name in SAMPLE:
                ...
                value = str(sheet.cell(row, 20).value) if sheet.cell(row, 20).value else ""
                if "." in value:
                    value = float(value)
                else:
                    value = 0
