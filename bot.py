...
async def up_guild_list():
    ...
    while (1):
        if guild_list != [] and len(guild_list_temp) >= 100:
            guild_list_temp = await api.me_guilds(option=ReqOption(
                after=str(guild_list_temp[-1].id)))
...
async def check_cookies():
    ...
    for i in load_dict['data']:
        ...
        mysid = mysid_data[1]
        cookie = ';'.join(
            filter(lambda x: x.split('=')[0] in ['cookie_token', 'account_id'],
...
async def check_startwish(raw_mes, key_word, gid):
    if raw_mes.startswith(key_word) and await check_switch(
            gid, Config.switch_list[key_word]):
        return True
    else:
        return False
...
async def check_endwish(raw_mes, key_word, gid):
    if raw_mes.endswith(key_word) and await check_switch(
            gid, Config.switch_list[key_word]):
        return True
    else:
        return False
    ...
    try:
        while 1:
            ...
            if raw_data['retcode'] != 0:
                ...
                break
        style = 'egenshin'
        url = 'https://yuanshen.minigg.cn/generator/user_info?style=' + style + '&uid=' + uid + '&qid=' + qid + '&nickname=' + urllib.parse.quote(
            nickname) + '&quality=75'
        async with AsyncClient() as client:
            req = await client.post(url=url, json=raw_data)
        return req.text
...
async def _message_handler(event, message: Message):
    qqbot.logger.info('event %s' % event +
                      ',receive message %s' % message.content)

    try:
        guild_data = await guild_api.get_guild(message.guild_id)
        at_mes = re.search(r'<@![0-9]+>', message.content)
        raw_mes = message.content.replace(at_mes.group(),
                                          '').replace(' ',
                                                      '').replace('/',
                                                                  '').strip()
        record_mes = raw_mes
    ...
    if raw_mes.startswith('开启'):
        ...
        member_info = await guild_member_api.get_guild_member(
            message.guild_id, message.author.id)
        if '2' in member_info.roles or '4' in member_info.roles:
            try:
                await change_switch(message.guild_id,
                                    Config.switch_list[raw_mes], 'on')
                mes = '成功。'
            except Exception as e:
                traceback.print_exc()
        ...
        if '2' in member_info.roles or '4' in member_info.roles:
            try:
                await change_switch(message.guild_id,
                                    Config.switch_list[raw_mes], 'off')
                mes = '成功。'
            except Exception as e:
                traceback.print_exc()
                mes = '发生错误，可能是输入的功能名不正确。'
            ...
            if '2' in member_info.roles or '4' in member_info.roles:
                ...
                for i in channel_list:
                    ...
                    if temp_name == channel_name:
                        channel_id = i.id
                await change_subGuild_switch(message.guild_id, channel_id,
                                             'open')
    ...
    if not await check_subGuild_switch(message.guild_id, message.channel_id):
        ...
        if raw_mes == '频道信息':
            ...
            try:
                ...
                qqbot.logger.info(e.with_traceback)
                mes = 'ck添加错误。'
        elif raw_mes == '当前状态' and await check_switch(
                message.guild_id, Config.switch_list['当前状态']):
            try:
                ...
                traceback.print_exc()
        elif raw_mes == '每月统计' and await check_switch(
                message.guild_id, Config.switch_list['每月统计']):
            try:
                uid = await select_db(message.author.id, mode='uid')
                uid = uid[0]
                mes = await award(uid)
                ...
                mes = '发生错误 {},请检查后台输出。'.format(e)
                traceback.print_exc()
        elif raw_mes == '签到' and await check_switch(message.guild_id,
                                                    Config.switch_list['签到']):
            try:
                uid = await select_db(message.author.id, mode='uid')
                uid = uid[0]
                mes = await sign(uid)
            ...
            try:
                ...
                if name == '列表':
                    ...
                    audio_img = 'https://img.genshin.minigg.cn/avatar/{}.png'.format(
                        name)
        ...
        elif raw_mes == '查询' and await check_switch(message.guild_id,
                                                    Config.switch_list['查询']):
            try:
                uid = await select_db(message.author.id)
                author = await guild_member_api.get_guild_member(
                    message.guild_id, message.author.id)
                nickname = author.user.username
                get_url = await GetUidUrl(uid[0], message.author.id, nickname,
                                          uid[1])
            ...
            try:
                ...
                if len(uid) != 9:
                    ...
                    get_url = await GetUidUrl(uid, message.author.id,
        ...
        #    pass
            ...
            try:
                name = ''.join(re.findall('[\u4e00-\u9fa5]', raw_mes))
                image = 'https://img.genshin.minigg.cn/guide/{}.jpg'.format(
            ...
            try:
                name = ''.join(re.findall('[\u4e00-\u9fa5]', raw_mes))
                image = 'https://img.genshin.minigg.cn/info/{}.jpg'.format(
                    urllib.parse.quote(name, safe=''))
                status = httpx.get(url=image).text
                if '404 Not Found' in status:
                    image = None
        ...
        elif raw_mes == '御神签' and await check_switch(
                message.guild_id, Config.switch_list['御神签']):
            try:
    ...
    if ark:
        try:
            send = qqbot.MessageSendRequest(content='',
                                            ...
                                            msg_id=message.id)
            await msg_api.post_message(message.channel_id, send)
            await record(guild_data.name, message.guild_id,
...
async def _dms_handler(event, message: Message):
    ...
    if mes:
        try:
            msg_request = qqbot.MessageSendRequest(content=mes,
                                                   msg_id=message.id)
            msg = await dms_api.post_direct_message(message.guild_id,
                                                    msg_request)
            await record('私信', message.guild_id, message.author.username,
                         message.author.id, record_mes, mes)
        except Exception as e:
            traceback.print_exc()
            await record('私信', message.guild_id, message.author.username,
                         message.author.id, record_mes, str(e))
...
async def getGuildStatus():
    ...
    try:
        ...
        traceback.print_exc()
    user = await api.me()
    guild_status_mes = '【{}】总加入频道 {} 个,总人数为 {}'.format(
        user.username, str(len(guild_list)), str(guild_member_all_count))
    return guild_status_mes


qqbot_guildevent_handler = qqbot.Handler(qqbot.HandlerType.GUILD_EVENT_HANDLER,
                                         _guild_handler)
qqbot_atmessage_handler = qqbot.Handler(
    qqbot.HandlerType.AT_MESSAGE_EVENT_HANDLER, _message_handler)
qqbot_dms_handler = qqbot.Handler(
    qqbot.HandlerType.DIRECT_MESSAGE_EVENT_HANDLER, _dms_handler)
qqbot.async_listen_events(token, False, qqbot_guildevent_handler,
                          qqbot_atmessage_handler, qqbot_dms_handler)
