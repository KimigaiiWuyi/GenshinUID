...
async def get_a_lots(qid):
    ...
    if len(c_data) == 0:
        ...
        if c_data[0][1] is None:
            ...
            data = raw_data[num]
            c.execute('UPDATE UseridDict SET lots = ? WHERE QID=?',
                      (str(num), qid))
        else:
            num = int(c_data[0][1])
            data = raw_data[num]
...
async def open_push(uid, qid, status, mode):
    ...
    if len(c_data) != 0:
        try:
            c.execute(
                'UPDATE NewCookiesTable SET {s} = ?,QID = ? WHERE UID=?'.
                format(s=mode), (status, qid, uid))
            conn.commit()
            conn.close()
            return '成功！'
        except:
...
async def check_db():
    ...
    for row in c_data:
        try:
            ...
            try:
                c.execute('DELETE from CookiesCache where Cookies=?',
                          (row[1], ))
            except:
                pass
    if len(c_data) > 9:
...
async def delete_cache():
    ...
    try:
        ...
        c.execute('UPDATE NewCookiesTable SET Extra = ? WHERE Extra=?',
                  (None, 'limit30'))
...
def cache_db(uid, mode=1, mys=None):
    ...
    if mode == 1:
        if mys:
            cursor = c.execute('SELECT *  FROM CookiesCache WHERE MYSID = ?',
                               (mys, ))
        else:
            cursor = c.execute('SELECT *  FROM CookiesCache WHERE UID = ?',
                               (uid, ))
    else:
        cursor = c.execute('SELECT *  FROM CookiesCache WHERE MYSID = ?',
                           (uid, ))
    ...
    if len(c_data) == 0:
        if mode == 2:
            ...
            cursor = c.execute('SELECT *  FROM NewCookiesTable WHERE UID = ?',
                               (uid, ))
            d_data = cursor.fetchall()
        ...
        if mys:
            try:
                c.execute('UPDATE CookiesCache SET UID = ? WHERE MYSID=?',
                          (uid, mys))
            except:
                c.execute('UPDATE CookiesCache SET MYSID = ? WHERE UID=?',
                          (mys, uid))
...
async def cookies_db(uid, cookies, qid):
    ...
    if len(c_data) == 0:
        ...
        c.execute('UPDATE NewCookiesTable SET Cookies = ? WHERE UID=?',
                  (cookies, uid))
...
async def stoken_db(s_cookies, uid):
    ...
    c.execute('UPDATE NewCookiesTable SET Stoken = ? WHERE UID=?',
              (s_cookies, int(uid)))
        ...
        try:
            async with AsyncClient() as client:
                req = await client.get(
                    ...
                    headers={
                        'DS':
                        get_ds_token('role_id=' + uid + '&server=' +
                                     server_id),
                        'x-rpc-app_version':
                        mhyVersion,
                        'User-Agent':
...
async def get_spiral_abyss_info(uid,
                                ck,
                                schedule_type='1',
                                server_id='cn_gf01'):
    if uid[0] == '5':
            ...
            req = await client.get(
                ...
                headers={
                    ...
                    get_ds_token('role_id=' + uid + '&schedule_type=' +
                                 schedule_type + '&server=' + server_id),
                    'Origin':
                    'https://webstatic.mihoyo.com',
                    'Cookie':
                    ck,
                    ...
                    mhyVersion,
                    'User-Agent':
                    'Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS '
...
async def get_calculate_info(client: ClientSession,
                             uid,
                             char_id,
                             ck,
                             name,
    ...
    req = await client.get(
        ...
        headers={
            'DS':
            get_ds_token('uid={}&avatar_id={}&region={}'.format(
                uid, char_id, server_id)),
            'x-rpc-app_version':
            mhyVersion,
            'User-Agent':
            'Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 ('
    ...
    try:
        ...
        try:
            async with AsyncClient() as client:
                req = await client.get(
                    url=
                    'https://api-takumi-record.mihoyo.com/game_record/card/wapi/getGameRecordCard?uid='
                    + mysid,
                    headers={
                        'DS':
                        get_ds_token('uid=' + mysid),
    ...
    async with AsyncClient() as client:
        req = await client.get(
            ...
            headers={
                ...
                'Referer':
                'https://genshin.minigg.cn/index.html'
            },
            params={
                'characters': name,
                'audioid': audioid,
